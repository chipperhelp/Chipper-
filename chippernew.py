...
with placeholder.container():
    ...
    """
     ...
     streamlitstyle = """
			<style>
			sidebar,body,[class*="css"]{background-image: url("https://media.tenor.com/SpNcK_OnyC0AAAAC/water-wave.gif");
...
#https://64.media.tumblr.com/0282981b3d64c161dfee6db1fcba9b2e/c6c844929d40597e-5c/s500x750/68afbb70ac6ec01b1c2819383b7bedd09fd1da7a.gif
#https://dashboard.render.com/static/media/blob-baby-blue.7868befcc1c6a66edc5ae9ddbb055ccb.svg
...
if signin:
       ...
       # "nav-link-selected": {"background-color": "lightblue","color":"#1c1c1c"},})
                    ...
                    menu_data = [{'id':'    ','icon': "bi bi-house-fill", 'label':"Home"},{'id':' ','icon': "bi bi-search", 'label':"Search"},{'id':'  ','icon': "bi bi-pencil-square", 'label':"Share"},{'id':'   ','icon': "bi bi-person-fill", 'label':f"{userdata}"}]
                    ...
                    nimg=data.child(user["localId"]).child("Images").get().val()#padding must be 10 if not adding chat
                    ...
                    if nav=="   ":
                          ...
                          with tab4:
                              ...
                              if  posts.val() is not None:
                                   ...
                                   st.markdown(f"___")
                              with tab5:
                                   posts=data.child(user['localId']).child("videosdata").get()
                                   if  posts.val() is not None:
                                        for Posts in reversed(posts.each()):
                                             caption = Posts.val()["caption"]
                                             image_url = Posts.val()["image_url"]
                                        ...
                                        with expn:
                                             ...
                                             if up:
                                                  with st.spinner("Uploading your video..."):
                                                       ...
                                                       userd=data.child(user["localId"]).child("Handle").get().val()
                                                       now=datetime.now()
                                                       dt=now.strftime("%d / %m / %y")
                                                       dtt=now.strftime("%I:%M %p")
                                                       captiondata=f'''{userd} \n\n  {cap} \n\n shared on : {dt} at  {dtt}'''
                                                       post_data = {"caption": captiondata,"image_url": storage.child("videos/" + vidx.name).get_url(None) }
                                                       data.child(user["localId"]).child("videosdata").push(post_data)
